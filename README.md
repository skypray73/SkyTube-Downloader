...
前往 [Releases](../../releases/latest) 下載 `SkyTubeDownloader.exe`。這是約 100 MB 的單一執行檔，內含：
...
第一次執行 `build.py` 會從已安裝的 Python 套件取出 FFmpeg 和 Deno，建立本機 `tools` 目錄，再產生 `dist\SkyTubeDownloader.exe`。
